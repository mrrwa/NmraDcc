...
uint16_t getMyAddr (void)
{
    ...
    if (DccProcState.cv29Value & CV29_ACCESSORY_DECODER)   // Accessory Decoder?
    {
        ...
        {
            DccProcState.myDccAddress = ( (readCV (CV_ACCESSORY_DECODER_ADDRESS_MSB) & 0b00000111) << 6) | (readCV (
                                            CV_ACCESSORY_DECODER_ADDRESS_LSB) & 0b00111111) ;
        }
    }
    else   // Multi-Function Decoder?
    {
        ...
        {
            DccProcState.myDccAddress = ( (readCV (CV_MULTIFUNCTION_EXTENDED_ADDRESS_MSB) - 192) << 8) | readCV (
...
void processDirectCVOperation (uint8_t Cmd, uint16_t CVAddr, uint8_t Value, void (*ackFunction) ())
{
    ...
    else
    {
        ...
        DB_PRINT ("CV: %d Current Value: %02X  Bit-Wise Mode: %s  Mask: %02X  Value: %02X", CVAddr, tempValue, BitWrite ? "Write":"Read", BitMask,
                  BitValue);
